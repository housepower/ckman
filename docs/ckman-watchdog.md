...
`bin/ckman-watchdog` 是 ckman 的自愈守护程序，由 crontab 每分钟调起、一次性运行。
...
- 依赖探测零副作用：`local` 策略不探 DB；`mysql/postgres/dm8` 与 Nacos 仅 TCP 拨号；不读密码、不连库迁移。故同一 ckman-watchdog 二进制可跨 ckman 版本独立分发。
...
```cron
* * * * * /path/to/ckman/bin/ckman-watchdog >/dev/null 2>&1
```

`flock`(`run/ckman-watchdog.lock`)保证不会叠跑。

## 日志

- `logs/ckman-watchdog.log`：每轮探测的 verdict 与动作。
- `logs/ckman-alert.log`：`[HEAL]/[ALERT]/[CRIT]/OK` 审计痕迹。

## 手动探测(接入外部监控)

```bash
bin/ckman-watchdog -check    # 仅探测,不自愈;退出码 0=健康 10=DOWN 11=HUNG 12=APP/异常 3=鉴权
```
...
ckman-watchdog 照 `bin/start` 风格自推导：以二进制自身位置 `bin/ckman-watchdog` 的上级目录为 HDIR，据此定位 `conf/ckman.hjson`、`run/ckman.pid`、`logs/`、`run/ckman-watchdog.{lock,restarts}`。可用 `-c` / `-pid` 覆盖。
