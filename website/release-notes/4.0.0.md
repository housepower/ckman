...
> 【封面图占位 · `cover-4.0.0.png`】建议放一张 CKMAN 4.0 主界面 + "4.0.0" 字样的视觉图
...
> 【截图占位 · `backup-policy-list.png`】备份策略列表页（含 daily / full / partition 模式）
>
> 【截图占位 · `backup-run-detail.png`】Run 详情：partition 级状态、checksum、耗时
...
> 【截图占位 · `user-mgmt-list.png`】用户管理列表
>
> 【截图占位 · `user-mgmt-create.png`】新建用户弹窗
...
> 【截图占位 · `node-override-dialog.png`】单节点 override 弹窗（XML 编辑器 + XPath 路径预览）
...
> 【截图占位 · `nacos-hot-reload.png`】Nacos 控制台改配置 → ckman 日志中显示 OnChange 触发链路
...
> 【截图占位 · `ckmanctl-sql-repl.gif`】REPL 交互演示
...
> 【截图占位 · `task-step-progress.png`】任务详情现在显示 Current Step
>
> 【截图占位 · `rebalance-plan-preview.png`】Rebalance Plan 预览向导
...
> 【截图占位 · `docs-vitepress-home.png`】文档站首页
>
> 【截图占位 · `download-page.png`】下载页（含 4.0.0 release notes 内嵌）
...
> 【尾图占位 · `footer-qrcode.png`】公众号二维码 / 项目二维码
